...
plot_global_otu <- function(res_sub, ylab1="", ylab2="", ylab3="") {
    ...
    plot(NA, main="", xlab="", ylab=ylab1, ylim=c(0.05, 0.9), xlim=c(beta_range1[1]-d1, beta_range1[2]+d1), bty="n")
    box(lwd=1.5)
    ...
    points(res_sub$beta[sub1], res_sub[sub1, 3], col="red", type="o", lwd=1.5, lty=2, pch=16)
    points(res_sub$beta[sub1], res_sub[sub1, 4], col="red", type="o", lwd=1.5, lty=1, pch=16)
    points(res_sub$beta[sub1], res_sub[sub1, 5], col="black", type="o", lwd=1.5, lty=1, pch=4)
    
    sub2 <- c(4:8)
    beta_range1 = range(res$beta[sub2])
    d1=(beta_range1[2]-beta_range1[1])*0.03
    plot(NA, main="", xlab="", ylab=ylab2, ylim=c(0.05, 0.65), xlim=c(beta_range1[1]-d1, beta_range1[2]+d1), bty="n")
    box(lwd=1.5)
    points(res_sub$beta[sub2], res_sub$pcom_sen[sub2]-0.02, col="green3", type="o", lwd=1.5, lty=1, pch=17)
    points(res_sub$beta[sub2], res_sub$pcomHM_sen[sub2]-0.02, col="green3", type="o", lwd=1.5, lty=2, pch=25)
    ...
    if (causal_type==1) legend(x=beta_range1[1]-d1, y=1, 
                               c("New-omni", "New-equal", "New-count", "Com-p-C", "Com-p-HM", "16S", "SMS"), 
                               col=c("black", "red", "red", "green3", "green3", "blue", "lightblue"), 
                               pch=c(4, 16, 16, 17, 25, 15, 19),
                               lty=c(1, 1, 2, 1, 2, 1, 1),
