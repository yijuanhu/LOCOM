...
Com2seq.ind <- function(table1, table2, Y1, Y2, C1 = NULL, C2 = NULL,
    ...
    # cols_with_0_5_indices==ref.taxon # true (ref taxa does not execute the Newton algorithm.")
    n.X <- ncol(X)
    XX <- CalculateXX(X)
    if (!we.only) weight.count = (table.comTaxa.filter + table.comTaxa.filter[, ref.taxon])
    if(we) weight.equal = matrix(rep(colMeans(table.comTaxa.filter + table.comTaxa.filter[, ref.taxon]), n.sam.com),
                          ncol=n.taxa.common.filter, byrow=TRUE) # temporary
    ...
    # # -----------------------
    # save the not idices that not converge
    Not_Converge_Taxa = which(res.obs.wc$final_step_sum < 3) #consider the reference taxon's influence of the indices
...
# Function Definition -----------------------------------------------------
    ...
    if (!we.only) {
        parallel.perm.wc <- function(i) {
          res.perm.wc.fun <- perm_Newton(freq_table = freq.table.ref, Yr = Yr, X = X, XX = XX,
    ...
    if (we){
     parallel.perm.we <- function(i) {
       res.perm.we.fun <- perm_Newton(freq_table = freq.table.ref, Yr = Yr, X = X, XX = XX,
...
# For Loop to implement permutation ----------------------------------------------------------------
    for (i.block in 1:n.block) {
        ...
        if (n.cores > 1) {
            ...
            if (we) res.perm.we <- abind(parallel.stat.we)
            ...
            if (!we.only) {
                res.perm.wc <- perm_Newton(freq_table = freq.table.ref, Yr = Yr, X = X, XX = XX,
                                                        ...
                                                        perm = perm.mat,
                                                        tol = tol, iter_max = iter_max, Firth_thresh = Firth.thresh, robust_var = robust_var,
                                                        prop_presence = prop.presence, get_var = FALSE)$beta
            }
            if (we){
            ...
            if (n.taxa1.only.filter > 0) res.perm1 <- perm_Newton(freq_table = freq.table1.ref, Yr = Y1r, X = X1, XX = XX1,
                                               ...
                                               perm = perm.mat1,
                                               tol = tol, iter_max = iter_max, Firth_thresh = Firth.thresh, robust_var = FALSE,
                                               prop_presence = prop.presence1, get_var = FALSE)$beta
            if (n.taxa2.only.filter > 0) res.perm2 <- perm_Newton(freq_table = freq.table2.ref, Yr = Y2r, X = X2, XX = XX2,
                                               beta_init = beta_init2, weight = weight2,
                                               perm = perm.mat2,
                                               tol = tol, iter_max = iter_max, Firth_thresh = Firth.thresh, robust_var = FALSE,
                                               prop_presence = prop.presence2, get_var = FALSE)$beta
        }
